...
def main(argv):
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="the batch size of generating"
    )
    ...
    for ii in range(0, args.n_sequences, args.batchsz):
        floor_plan = []
        tr_floor = []
        room_mask = []
        current_scenes = []
        ...
        for i in range(ii, ii + args.batchsz):
            if args.fix_order:
                if i < len(dataset):
        ...
        sv = torch.load(args.output_directory + '/' + args.path_to_source_pt + ".pt")
        sv = dataset.pre_process(sv, transform=True)

        if not config["validation"]["gen_traj"]:        
            bbox_params_traj = network.optimize_layout(
                    ...
                    text=[samples['description'] for samples in sampless] if 'description' in samples.keys() else None,
                    device=device,
                    freq=1, start_value=sv.to(device), start_step=0, end_step=-100,
                    clip_denoised=args.clip_denoised,
                    wallTensor =torch.tensor([samples['wa'] for samples in sampless],device=device) if 'wa' in samples.keys() else None,
                    windoorTensor = torch.tensor([samples['wd'] for samples in sampless],device=device) if 'wd' in samples.keys() else None,
                    batch_seeds=torch.arange(ii, ii+args.batchsz),
            )
            ...
            for k_time, v_time in bbox_params_traj.items():
                ...
                for i in range(args.batchsz):
                    boxes = dataset.post_process(bbox_params[i])
                    ...
                    bbox_params_traj[k_time][i] = bbox_params_t

                    path_to_pt = "{}/{}/{}_{}_{:03d}_time{:04d}.pt".format(
                        args.output_directory,
                        args.path_to_source_pt,
                        current_scenes[i].scene_id,
                        scene_idxes[i],
                        i+ii,
                        k_time
                    )
                    torch.save(bbox_params_t, path_to_pt)
