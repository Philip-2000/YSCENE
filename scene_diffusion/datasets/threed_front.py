...
class CachedThreedFront(ThreedFront):
    def __init__(self, base_dir, config, scene_ids):
        ...
        self._distance_matrix = config.get("distance_matrix", False)
        self._process_wall = config.get("process_wall", False)
        self._process_windoor = config.get("process_windoor", False)
    ...
    def __getitem__(self, i):
        ...
        if self._process_wall:
            ...
            for p in range(len(WA["contour"])):
                WA["contour"][p][0] -= D["floor_plan_centroid"][0]
                WA["contour"][p][1] -= D["floor_plan_centroid"][2]
        if self._process_windoor:
            WD = np.load(self._path_to_rooms[i][:-9] + "conts.npz")
            for p in range(len(WD["cont"])):
    ...
    def _parse_train_stats(self, train_stats):
        ...
        self._max_wall = self.config.get('max_wall', 12)
        self._max_windoor = self.config.get('max_windoor', 8)
    ...
    @property
    def max_windoor(self):
        return self._max_windoor 
