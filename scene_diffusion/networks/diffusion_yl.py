...
class GaussianDiffusion:
    def __init__(self, config, betas, loss_type, model_mean_type, angle_normalize, rotate_translation, model_var_type, loss_separate, loss_iou, train_stats_file):
    ...
    def q_sample_full(self, x_start, t):
        noise = torch.randn(x_start.shape, device=x_start.device)
        if self.rotate_translation and self.angle_normalize and self.angle_dim == 2:
            x_t = (
                self._extract(self.sqrt_alphas_cumprod.to(x_start.device), t, x_start.shape) * x_start +
                self._extract(self.sqrt_one_minus_alphas_cumprod.to(x_start.device), t, x_start.shape) * noise
            )

            #normalize its angle dimension of x_t

            x_start_a = x_start[:,:,self.bbox_dim-self.angle_dim:self.bbox_dim]
            x_t_a = x_t[:,:,self.bbox_dim-self.angle_dim:self.bbox_dim]
            x_t_an = F.normalize(x_t_a, dim=-1)
            noise_an =  ( x_t_an - self._extract(self.sqrt_alphas_cumprod.to(x_t_a.device), t, x_t_a.shape)*x_start_a ) / self._extract(self.sqrt_one_minus_alphas_cumprod.to(x_t_a.device), t, x_t_a.shape)
            
            noise_zn = x_t_an[:,:,:1] * noise[:,:,2:3] - x_t_an[:,:,1:] * noise[:,:,:1]
            noise_xn = x_t_an[:,:,1:] * noise[:,:,2:3] + x_t_an[:,:,:1] * noise[:,:,:1]
            noise_xyzn = torch.cat([noise_xn, noise[:,:,1:2], noise_zn], dim=-1)

            x_t_xyzn = (
                self._extract(self.sqrt_alphas_cumprod.to(noise_xyzn.device), t, noise_xyzn.shape) * x_start[:,:,:3] +
                self._extract(self.sqrt_one_minus_alphas_cumprod.to(noise_xyzn.device), t, noise_xyzn.shape) * noise_xyzn
            )

            noise = torch.cat([           noise[:,:,0:self.bbox_dim-self.angle_dim],noise_an,noise[:,:,self.bbox_dim:]], dim=-1)
            x_t   = torch.cat([  x_t_xyzn,  x_t[:,:,3:self.bbox_dim-self.angle_dim],  x_t_an,  x_t[:,:,self.bbox_dim:]], dim=-1)
            
            return x_t, noise
        elif self.angle_normalize and self.angle_dim == 2:
            x_t = (
                self._extract(self.sqrt_alphas_cumprod.to(x_start.device), t, x_start.shape) * x_start +
                self._extract(self.sqrt_one_minus_alphas_cumprod.to(x_start.device), t, x_start.shape) * noise
            )
            ...
            #normalize its angle dimension of x_t
            x_start_zx = torch.cat([x_start[:,:,self.translation_dim-1:self.translation_dim], x_start[:,:,:1]], dim=-1)
            ...
            noise = torch.cat([noise[:,:,:self.bbox_dim-self.angle_dim],noise_an,noise[:,:,self.bbox_dim:]], dim=-1)
            x_t   = torch.cat([  x_t[:,:,:self.bbox_dim-self.angle_dim],  x_t_an,  x_t[:,:,self.bbox_dim:]], dim=-1)
    ...
    def p_mean_variance(self, denoise_fn, data, t, condition, condition_cross, clip_denoised: bool, return_pred_xstart: bool, wallTensor=None, windoorTensor=None):

        model_output = denoise_fn(data, t, condition, condition_cross, wallTensor, windoorTensor)
        ...
        if self.model_mean_type == 'eps':
            if self.rotate_translation and self.angle_normalize and self.angle_dim == 2:
                #translate the model output's xyz from data_an to world_an
                data_a = data[:,:,self.bbox_dim-self.angle_dim:self.bbox_dim]
                data_an = F.normalize(data_a, dim=-1)
                
                mo_zn = data_an[:,:,:1] * model_output[:,:,2:3] + data_an[:,:,1:] * model_output[:,:,:1]
                mo_xn =-data_an[:,:,1:] * model_output[:,:,2:3] + data_an[:,:,:1] * model_output[:,:,:1]
                mo_xyzn = torch.cat([mo_xn, model_output[:,:,1:2], mo_zn], dim=-1)

                mo   = torch.cat([  mo_xyzn,  model_output[:,:,3:]], dim=-1)
                x_recon = self._predict_xstart_from_eps(data, t=t, eps=mo)
            else:
    ...
    def p_sample(self, denoise_fn, data, t, condition, condition_cross, noise_fn, clip_denoised=False, return_pred_xstart=False, wallTensor=None, windoorTensor=None):
        """
        ...
        model_mean, _, model_log_variance, pred_xstart = self.p_mean_variance(denoise_fn, data=data, t=t, condition=condition, condition_cross=condition_cross, clip_denoised=clip_denoised,
                                                                 return_pred_xstart=True, wallTensor=wallTensor, windoorTensor=windoorTensor)
        ...
        if self.angle_normalize and self.angle_dim == 2:
            sample_an = F.normalize(sample[:,:,self.bbox_dim-self.angle_dim:self.bbox_dim], dim=-1)
            sample = torch.cat([sample[:,:,:self.bbox_dim-self.angle_dim], sample_an, sample[:,:,self.bbox_dim:]], dim=-1)
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None, condition=None, condition_cross=None, wallTensor=None, windoorTensor=None, disMat=None):
        ...
        data_t, noise = self.q_sample_full(x_start=data_start, t=t)
        ...
        if self.loss_type == 'mse':
            ...
            denoise_out = denoise_fn(data_t, t, condition, condition_cross, wallTensor, windoorTensor)
            ...
            if self.model_mean_type == 'eps':
                raise NotImplementedError
                x_recon = self._predict_xstart_from_eps(data, t=t, eps=denoise_out)
            elif self.model_mean_type == 'x0':
                target = x_recon
            else:
                raise NotImplementedError
            ...
            #get the distance matrix of x_recon 
            #losses = ((target - disMat)**2) base on the values in disMat, nearer distance is 
        else:
            raise NotImplementedError(self.loss_type)
...
#this so-called "DiffusionPoint" combined the "GaussianDiffusion" and the "denoise_net" together
#the "GaussianDiffusion" calculate the losses between two "sample_param"s           the "denoise_net" reconstruct the sample_param from a noisy one (usually UNet1D)
class DiffusionPoint(nn.Module):
    def __init__(self, denoise_net, config, schedule_type='linear', beta_start=0.0001, beta_end=0.02, time_num=1000, 
            loss_type='mse', model_mean_type='eps', angle_normalize=False, rotate_translation=False, model_var_type ='fixedsmall', loss_separate=False, loss_iou=False, train_stats_file=None):
          
        super(DiffusionPoint, self).__init__()
        betas = get_betas(schedule_type, beta_start, beta_end, time_num)
        self.diffusion = GaussianDiffusion(config, betas, loss_type, model_mean_type, angle_normalize, rotate_translation, model_var_type, loss_separate, loss_iou, train_stats_file)
        self.process_wall = config.get("process_wall", False)
        self.process_windoor = config.get("process_windoor", False)
        self.independent_wall = config.get("independent_wall", False)
        self.independent_windoor = config.get("independent_windoor", False)
        self.rel_wall_dim = config["net_kwargs"].get("rel_wall_dim", 0)
        self.rel_windoor_dim = config["net_kwargs"].get("rel_windoor_dim", 0)
        self.loss_type = loss_type
        self.rotate_translation = config["diffusion_kwargs"].get("rotate_translation", False)
        self.ind_dim = config["net_kwargs"].get("ind_dim", 0)
        self.model = Unet1DYL(**config["net_kwargs"])  #denoise_net
    ...
    def _denoise(self, data, t, condition, condition_cross, wallTensor, windoorTensor):
        B,D,N= data.shape
        #assert data.dtype == torch.float
        assert t.shape == torch.Size([B]) and t.dtype == torch.int64

        if self.ind_dim > 0 or self.process_wall or self.process_windoor:
            res, resWall, resWindoor = self.rela_calculator.newRela(data, wallTensor, windoorTensor)


            if self.process_wall and self.process_windoor:
                resWall = torch.tensor(resWall,dtype=torch.float)
                _,__,c = resWall.shape
                assert c == self.rel_wall_dim
                resWindoor = torch.tensor(resWindoor,dtype=torch.float)
                _,__,c = resWindoor.shape
                assert c == self.rel_windoor_dim
                #resWall = torch.zeros_like(resWall)
                #resWindoor = torch.zeros_like(resWindoor)
                rdata = torch.cat([data, resWall, resWindoor], axis=-1)
            elif self.process_wall:
                resWall = torch.tensor(resWall,dtype=torch.float)
                _,__,c = resWall.shape
                assert c == self.rel_wall_dim
                rdata = torch.cat([data, resWall], axis=-1)
            elif self.process_windoor:
                resWindoor = torch.tensor(resWindoor,dtype=torch.float)
                _,__,c = resWindoor.shape
                assert c == self.rel_windoor_dim
                rdata = torch.cat([data, resWindoor], axis=-1)
            else:
                rdata = data
            ...
            if self.ind_dim == 0:
                ...
                _,__,___,c = res.shape
                assert c == self.ind_dim
                #res = torch.zeros_like(res)

            out = self.model(rdata, t, condition, condition_cross, res)
        else:
            if self.process_wall and self.process_windoor:
                if self.loss_type != "x0":
                    raise NotImplementedError
                rela_t = self.rela_calculator.rela(data, wallTensor, windoorTensor)
                out = self.model(torch.tensor(rela_t,dtype=torch.float), t, condition, condition_cross)
                out = self.rela_calculator.recon(data, out)
            else:
