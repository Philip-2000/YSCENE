...
from .diffusion_yl import DiffusionPoint
...
class DiffusionSceneLayout_DDPM(Module):
    def __init__(self, n_classes, feature_extractor, config):
        ...
        if self.text_condition:
            ...
            if self.text_glove_embedding:
                ...
                self.tokenizer = BertTokenizer.from_pretrained('/home/storage/yueliang/DiffuScene-master/dataroot/models/bert-base-uncased')
                self.bertmodel = BertModel.from_pretrained("/home/storage/yueliang/DiffuScene-master/dataroot/models/bert-base-uncased")
            ...
            print('NOT use text as condition')

        # define the denoising network
        if config["net_type"] == "unet1d":
            denoise_net = None#Unet1D(**config["net_kwargs"])
        else:
    ...
    def sample(self, room_mask, num_points, point_dim, batch_size=1, text=None, 
               partial_boxes=None, input_boxes=None, ret_traj=False, ddim=False, clip_denoised=False, freq=40, batch_seeds=None, wallTensor=None, windoorTensor=None, 
                ):
    ...
    def generate_layout_progressive(self, room_mask, num_points, point_dim, batch_size=1, text=None, ret_traj=False, ddim=False, clip_denoised=False, wallTensor=None, windoorTensor=None, batch_seeds=None, device="cpu", keep_empty=False, num_step=100):
        ...
        samples_traj = self.sample(room_mask, num_points, point_dim, batch_size, text=text, ret_traj=ret_traj, ddim=ddim, clip_denoised=clip_denoised, wallTensor=wallTensor, windoorTensor=windoorTensor, batch_seeds=batch_seeds, freq=num_step)
        boxes_traj = {}
    ...
    def delete_empty_from_network_samples(self, samples, batch_size=1, device="cpu", keep_empty=False):
        ...
        for ii in range(batch_size):
            ...
            for i in range(max_boxes):
                ...
                if not keep_empty and samples_dict['objectness'][ii, i, -1] > 0:
                    continue
                else:
                    for k in samples_dict.keys():
                        if k == "class_labels":
                            # we output raw probability maps for visualization
                            boxes[k] = torch.cat([ boxes[k], samples[ii:ii+1, i:i+1, self.bbox_dim:self.bbox_dim+self.class_dim-1].to(device) ], dim=1)
                            boxes["objectness"] = torch.cat([ boxes["objectness"], samples[ii:ii+1, i:i+1, self.bbox_dim+self.class_dim-1:self.bbox_dim+self.class_dim].to(device) ], dim=1)
                        else:
                            boxes[k] = torch.cat([ boxes[k], samples_dict[k][ii:ii+1, i:i+1, :].to(device) ], dim=1)
    ...
    @torch.no_grad()
    def delete_empty_boxes(self, samples_dict, batch_size=1, device="cpu", keep_empty=False):
...
def train_on_batch(model, optimizer, sample_params, config):
    ...
    #print(loss)
    writer.add_scalar('loss/loss', loss, num_steps)
    writer.add_scalar('loss/bbox', StatsLogger.instance()["loss.bbox"].value, num_steps)
    writer.add_scalar('loss/trans', StatsLogger.instance()["loss.trans"].value, num_steps)
    writer.add_scalar('loss/size', StatsLogger.instance()["loss.size"].value, num_steps)
    writer.add_scalar('loss/angle', StatsLogger.instance()["loss.angle"].value, num_steps)
    writer.add_scalar('loss/class', StatsLogger.instance()["loss.class"].value, num_steps)
    loss.backward()
    # Compuite model norm
    ...
    writer.add_scalar('param/gradnorm', StatsLogger.instance()["gradnorm"].value, num_steps)
    # log learning rate
